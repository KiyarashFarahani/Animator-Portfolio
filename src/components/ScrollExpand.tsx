...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const responsiveRef = useRef({ startWidth, startHeight });
  responsiveRef.current = { startWidth, startHeight };
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    const res = responsiveRef.current;

    const e = smoothstep(0, 1, p);

    const w = res.startWidth + (100 - res.startWidth) * e;
    const h = res.startHeight + (100 - res.startHeight) * e;
    const ix = Math.max(0, (100 - w) / 2);
    const iy = Math.max(0, (100 - h) / 2);
    const radius = c.startRadius + (c.endRadius - c.startRadius) * e;
    frame.style.clipPath = `inset(${iy}% ${ix}% ${iy}% ${ix}% round ${radius}px)`;
  ...
  useEffect(() => {
    ...
    const measure = (force = false) => {
      ...
      if (!force && stageH > 0 && Math.abs(w - lastW) < 2) return;
      lastW = w;
      ...
      const mobileStartW = isMobile ? Math.max(c.startWidth, 65) : c.startWidth;
      const mobileStartH = isMobile ? Math.min(c.startHeight, 45) : c.startHeight;
      responsiveRef.current = { startWidth: mobileStartW, startHeight: mobileStartH };
      stage.style.setProperty('--se-start-w', `${mobileStartW}%`);
      stage.style.setProperty('--se-start-h', `${mobileStartH}%`);
    };
