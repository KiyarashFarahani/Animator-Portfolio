...
import { collectMedia, type MediaItem } from "@/lib/projects-server";
  ...
  const galleries = project.galleries
    ? await Promise.all(
        project.galleries.map(async (gallery) => ({
          ...gallery,
          media: await collectMedia(gallery.dir),
        }))
      )
    : null;
  const media = galleries ? [] : await collectMedia(project.dir);
